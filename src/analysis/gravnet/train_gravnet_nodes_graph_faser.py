...
def train_epoch(
    ...
    for batch_idx, data in enumerate(train_bar):
        ...
        try:
            ...
            if model.predict_charm:
                node_out, graph_out, charm_out = output
            else:
                node_out, graph_out = output
                charm_out = None
        ...
        if node_total > 0:
            ...
            postfix["NAcc"] = f"{current_node_acc:.2f}%"
            postfix["NWAcc"] = f"{current_node_wacc:.2f}%"
        ...
        if not sys.stdout.isatty() and (batch_idx + 1) % 100 == 0:
            ...
            if node_total > 0:
                log_msg += (
                    f", NAcc={current_node_acc:.2f}%, NWAcc={current_node_wacc:.2f}%"
                )
            if charm_total > 0:
                log_msg += (
    ...
    avg_node_loss = total_node_loss / graph_total
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Predict charm interaction flag in addition to graph/node classification",
    )
    ...
    weights_path = get_weights_path() / f"gravnet_nodes_graph_faser_{suffix}"
        ...
        for chunk in chunks_to_load:
            ...
            if chunk_file.exists():
                ...
                if run_str == "nut":
                    ...
                    dataset = [dataset[i] for i in hadronic_events_idx]
                    logging.info(
                        f"Selected {len(hadronic_event_ids)} hadronic tau events from {len(all_event_ids)} total events in chunk {chunk}"
                    )
    ...
    if has_node_labels:
        ...
        num_node_classes = 3  # Default assumption
        logger.warning(
    ...
    if has_node_labels:
        ...
        node_class_weights = torch.ones(num_node_classes, device=device)

    # Compute charm class weights (binary classification)
    charm_class_weights = None
    if args.predict_charm and hasattr(train_dataset[0], "charm"):
    ...
    # Create model with FASER features and node classification
    ...
    for epoch in range(args.num_epochs):
        ...
        metrics["train_node_loss"].append(train_node_loss)
        ...
        metrics["val_node_loss"].append(val_node_loss)
        ...
        log_msg = (
            ...
            f"Train Loss: {train_loss:.4f}, Node: {train_node_acc:.4f}/{train_node_wacc:.4f}, Graph: {train_graph_acc:.4f}/{train_graph_wacc:.4f} - "
            f"Val Loss: {val_loss:.4f}, Node: {val_node_acc:.4f}/{val_node_wacc:.4f}, Graph: {val_graph_acc:.4f}/{val_graph_wacc:.4f}"
        )
    ...
    logger.info(
        f"Final Val Loss: {val_loss_final:.4f} (Node: {val_node_loss_final:.4f}, Graph: {val_graph_loss_final:.4f}, Charm: {val_charm_loss_final:.4f})"
    )
    logger.info(
    ...
    # Node-level per-class accuracy (if available)
    ...
    if len(all_node_preds) > 0:
        ...
        num_node_classes_final = int(all_node_targets.max()) + 1
    ...
    save_dict = {
        ...
        "val_node_loss": val_node_loss_final,
