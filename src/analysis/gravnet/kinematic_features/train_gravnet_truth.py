...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=[10000, 10001, 10002, 10003],
    )
    parser.add_argument(
        "-c",
        "--chunks",
    ...
    output_path = get_torch_path() / f"gravnet_graph_faser_6_{suffix}"
    output_path.mkdir(parents=True, exist_ok=True)
    weights_path = get_weights_path() / f"gravnet_graph_faser_6_{suffix}"
    ...
    model_str = "gravnet_faser_truth"
    dataset = []
    for run in args.runs:
        run_str = get_str_from_run(run - 10000)
        run_path = torch_path / f"{run}/{model_str}"
        # Get chunks to load
        ...
        if run == 10002:
            chunks_to_load = list(range(10))
        for chunk in chunks_to_load:
            if run == 10002:
                for tau_decay in ["nut_e", "nut_mu", "nut_had"]:
                    chunk_file = run_path / f"{run_str}_{chunk:03d}_{tau_decay}.pt"
    ...
    logger.info(f"FASER features dim: {sample.x_event.shape[0]}")
    ...
    # gravnet_graph_faser_all_events_nut_5mm_0_scint_layer/

    # Create model with FASER features (graph-level classification only)
    print("input dim: ", sample.x.shape[1])
    print("event dim: ", sample.x_event.shape[0])
    print("num_graph_classes: ", num_graph_classes)
    model = NeutrinoGravNetTruth(
        input_dim=sample.x.shape[1],
        num_graph_classes=num_graph_classes,
        faser_dim=sample.x_event.shape[0],
