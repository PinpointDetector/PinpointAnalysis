...
from analysis.pointnetpp.model import NeutrinoPointNetPlusPlusGraphOnly
...
def train_epoch_graph(
    ...
    """Train one epoch with PointNet++ graph classification only.
...
def validate_epoch_graph(
    ...
    """Validate one epoch with PointNet++ graph classification only.
...
def main(
    ...
    """Train PointNet++ model for neutrino event classification (graph-level only)."""
    torch_path = get_torch_path()
    output_path = get_torch_path() / "pointnetpp_graph"
    output_path.mkdir(parents=True, exist_ok=True)
    weights_path = get_weights_path() / "pointnetpp_graph"
    ...
    model = NeutrinoPointNetPlusPlusGraphOnly(
    ...
    print("\nStarting PointNet++ graph-only training...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train PointNet++ for neutrino event classification (graph-level only)"
    )
