...
from analysis.pointnetpp.model import NeutrinoPointNet
...
def train_epoch_pointnet(
    ...
    """Train one epoch with PointNet graph classification.

    Returns:
        current_loss, graph_acc, graph_weighted_acc, final_loss
    """
    model.train()
    running_loss = 0
    graph_correct = 0
    ...
    for batch_idx, batch in enumerate(train_bar):
        ...
        # Forward pass
        graph_out = model(batch.x, batch.pos, batch.batch)

        # Compute graph-level loss
        if graph_class_weights is not None:
            loss = F.cross_entropy(graph_out, batch.y_graph, weight=graph_class_weights)
        else:
            loss = criterion(graph_out, batch.y_graph)
...
def validate_epoch_pointnet(
    ...
    """Validate one epoch with PointNet graph classification.

    Returns:
        avg_loss, graph_acc, graph_weighted_acc
    """
    model.eval()
    total_loss = 0
    graph_correct = 0
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_loader):
            ...
            try:
                ...
                total_loss += loss.item() * batch.y_graph.size(0)
    ...
    return avg_loss, graph_acc, graph_weighted_acc
...
def main(
    ...
    """Train PointNet model for neutrino event classification."""
    torch_path = get_torch_path()
    output_path = get_torch_path() / "pointnet"
    output_path.mkdir(parents=True, exist_ok=True)
    weights_path = get_weights_path() / "pointnet"
    ...
    # Calculate graph-level class weights
    ...
    lr = 1e-3  # PointNet is simpler and more stable than PointNet++
    weight_decay = 1e-4
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        ) = validate_epoch_pointnet(
            model, test_loader, criterion, device, graph_class_weights
        )
        ...
        print(
            f"Train Loss: {train_loss:.4f}, Graph: {train_graph_acc:.2f}% (W: {train_graph_weighted_acc:.2f}%) | "
            f"Val Loss: {val_loss:.4f}, Graph: {val_graph_acc:.2f}% (W: {val_graph_weighted_acc:.2f}%)"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train PointNet for neutrino event classification"
    )
