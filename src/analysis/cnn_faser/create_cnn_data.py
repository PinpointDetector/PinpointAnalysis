...
from analysis.utils.datasets import (
    CNN3DDataset,
    CNNProjectionDataset,
    CNNProjectionDatasetFaser,
)
...
def create_cnn_projection_torch_data(
    ...
    output_torch_path = (
        get_torch_path()
        / f"cnn_projection_faser_{pixel_size:03.0f}um_bins_10000_10003.pt"
    )
    output_truth_path = get_torch_path() / "cnn_projection_faser_truth.parq"
    ...
    nue_run = 10000
    nue_path = get_parquet_path() / f"{nue_run}/{pixel_size:03.0f}um_bins"
    nue_hits_files = sorted(nue_path.glob(f"{nue_run}_*_hits.parq"))[:num_files]
    nue_faser_files = sorted(nue_path.glob(f"{nue_run}_*_faser.parq"))[:num_files]
    nue_truth_files = sorted(nue_path.glob(f"{nue_run}_*_truth.parq"))[:num_files]
    nue_hits_df = pd.concat([pd.read_parquet(f) for f in nue_hits_files])
    nue_faser_df = pd.concat([pd.read_parquet(f) for f in nue_faser_files])
    nue_truth_df = pd.concat([pd.read_parquet(f) for f in nue_truth_files])

    num_run = 10001
    num_path = get_parquet_path() / f"{num_run}/{pixel_size:03.0f}um_bins"
    num_hits_files = sorted(num_path.glob(f"{num_run}_*_hits.parq"))[:num_files]
    num_faser_files = sorted(num_path.glob(f"{num_run}_*_faser.parq"))[:num_files]
    num_truth_files = sorted(num_path.glob(f"{num_run}_*_truth.parq"))[:num_files]
    num_hits_df = pd.concat([pd.read_parquet(f) for f in num_hits_files])
    num_faser_df = pd.concat([pd.read_parquet(f) for f in num_faser_files])
    num_truth_df = pd.concat([pd.read_parquet(f) for f in num_truth_files])

    nun_run = 10003
    nun_path = get_parquet_path() / f"{nun_run}/{pixel_size:03.0f}um_bins"
    nun_hits_files = sorted(nun_path.glob(f"{nun_run}_*_hits.parq"))[:num_files]
    nun_faser_files = sorted(nun_path.glob(f"{nun_run}_*_faser.parq"))[:num_files]
    nun_truth_files = sorted(nun_path.glob(f"{nun_run}_*_truth.parq"))[:num_files]
    nun_hits_df = pd.concat([pd.read_parquet(f) for f in nun_hits_files])
    nun_faser_df = pd.concat([pd.read_parquet(f) for f in nun_faser_files])
    ...
    nue_faser_df = nue_faser_df.loc[nue_faser_df["event_id"].isin(good_nue_event_ids)]
    ...
    num_faser_df = num_faser_df.loc[num_faser_df["event_id"].isin(good_num_event_ids)]
    ...
    nun_faser_df = nun_faser_df.loc[nun_faser_df["event_id"].isin(good_nun_event_ids)]
    nun_truth_df = nun_truth_df.loc[nun_truth_df["event_id"].isin(good_nun_event_ids)]

    if num_events is not None:
        nue_hits_df = nue_hits_df.query(f"event_id < {num_events}")
        nue_faser_df = nue_faser_df.query(f"event_id < {num_events}")
        nue_truth_df = nue_truth_df.query(f"event_id < {num_events}")
        num_hits_df = num_hits_df.query(f"event_id < {num_events}")
        num_faser_df = num_faser_df.query(f"event_id < {num_events}")
        num_truth_df = num_truth_df.query(f"event_id < {num_events}")
        nun_hits_df = nun_hits_df.query(f"event_id < {num_events}")
        nun_faser_df = nun_faser_df.query(f"event_id < {num_events}")
        nun_truth_df = nun_truth_df.query(f"event_id < {num_events}")

    data_list = [
        (nue_hits_df, nue_faser_df, nue_truth_df, 0),
        (num_hits_df, num_faser_df, num_truth_df, 1),
        (nun_hits_df, nun_faser_df, nun_truth_df, 2),
    ]

    mosaix_geo = Geometry()
    geo = get_rebinned_geometry(pixel_size=pixel_size * um, old_geometry=mosaix_geo)
    dataset = CNNProjectionDatasetFaser(
        data_list=data_list, bins=(num_bins, num_bins, num_layers), geo=geo
    )
    truth_df = pd.concat([nue_truth_df, num_truth_df, nun_truth_df], ignore_index=True)
